<script lang="ts">
  ...
  import { formatCommentInfo, formatDate, formatDateTime } from "../../utils/formatting";
  ...
  $: roleHeader =
    sortKey === "role" ? (sortAsc ? "ROLE ▼" : "ROLE ▲") : "ROLE&nbsp;&nbsp;";

  $: firstNameHeader =
    sortKey === "firstName" ? (sortAsc ? "Ime ▼" : "Ime ▲") : "Ime&nbsp;&nbsp;";
  ...
  $: zadnjiPutAktivanHeader =
    sortKey === "zadnjiPutAktivan"
      ? sortAsc
        ? "ZADNJI PUT AKTIVAN ▼"
        : "ZADNJI PUT AKTIVAN ▲"
      : "ZADNJI PUT AKTIVAN&nbsp;&nbsp;";
      ...
      >
        <tr class="h-12">
          ...
          <th
            ...
            on:click={() => sortBy("firstName")}
            >{@html firstNameHeader}
          </th>
          ...
          <th
            class="pgs-th cursor-pointer"
            title="Click to sort by"
            class:pgs-gradient-text={isActiveHeader("zadnjiPutAktivan")}
            class:dark\:bg-slate-700={isActiveHeader("zadnjiPutAktivan")}
            on:click={() => sortBy("zadnjiPutAktivan")}
            >{@html zadnjiPutAktivanHeader}
          </th>
      ...
      <tbody class="">
        {#each users as user, i}
          <tr class="bg-base-200/60 tr-highlight">
            ...
            <td>
              <span
                class="badge badge-soft badge-{getUserColor(
                  user.role
                )} badge-{getUserColor(user.role)} font-mono badge-sm"
            ...
            <td class="pgs-td"
              ><span class="block max-w-[200px] truncate" title={user.firstName}
                >{user.firstName}</span
              ></td
            >
            <td class="pgs-td"
              ><span class="block max-w-[200px] truncate" title={user.lastName}
                >{user.lastName}</span
              ></td
            >
            <td class="text-center">{@html formatCommentInfo(user.comment)}</td>
            <td class="pgs-td font-mono justify-right">
              {@html formatDateTime(user.zadnjiPutAktivan)}
            </td>
            <td class="pgs-td font-mono justify-right">
              {@html formatDate(user.created, "new", 15)}
            </td>
            <td class="pgs-td font-mono pgs-td-num">
              {@html formatDate(user.modified,
                "Izmenjeno pre manje od 15 minuta",
                15
              )}
            </td>
            ...
            <td class="px-2">
              ...
              >
                <div class="tooltip tooltip-info group" data-tip="Izmjeni">
                  <a
                    class="px-2"
                    aria-label="Izmjeni"
                    use:link
                ...
                <div class="tooltip tooltip-info group" data-tip="Obriši">
                  <button
                    class="px-2"
                    aria-label="Obriši"
                    on:click={() => deleteDialog(user.id)}
                  >
                    <i
        ...
        <tr class="bg-base-200">
          <td colspan="11" class="pgs-td font-mono h-[64px]"
            >Ukupno nadjeno korisnika: {users.length}</td
          >
        </tr>
      </tbody>
    </table>
