  ...
  async function handleSearch() {
    ...
    try {
      const res = await fetch(
        API_BASE_URL +
          `/barbacoa/products?search=${searchTerm}&page=${page}&size=${size}`,
        {
          method: "GET",
          headers: {
 ...
 -->              <span class="md:inline"> | Ukupno: <b>{products.length}</b></span>
  ...
  <div class="w-full overflow-x-auto">
              ...
              <tr class="bg-base-200/80 outline-1 outline-transparent /*hover:outline-blue-500*/ hover:bg-base-300/70 border-b border-b-base-300">            
                <td class="pgs-td whitespace-nowrap p-0">
                 <!--  <a
                  ...
                  > -->
                  <div class="">
                     <h3 class="font-semibold text-lg max-w-[382px;] truncate text-primary" title={i +'. '+ product.title}>
    ...
    <table class="w-full">
      <tbody>
        <tr>
         <td class="pgs-td text-left text-4xl "><h3 class="font-semibold text-lg truncate text-primary/50" >€ {product.priceBottle}</h3></td>
