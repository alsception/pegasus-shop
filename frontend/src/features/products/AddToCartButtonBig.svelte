...
{:else if $addedItems[product.id] > 0}  
    ...
    style="height: {height};width: {width};border-radius: 14px; padding:4px;justify-content:space-around;">
        <button 
...
{:else}
 <button
    class="btn btn-ghost bg-base-100 hover:bg-base-300 hover:text-primary dark:bg-secondary-content/10 dark:hover:border-2 group text-xl"
    style="width: {width}; height: {height};"
    aria-label="Dodaj u košaricu"
    on:click={() => addToCart(product.id)}
  >
    <i class="fa fa-cart-plus text-primary group-hover:text-info text-xl dark:hover:text-info" aria-hidden="true"></i>
    {#if full}
      Dodaj u košaricu
    {/if}
  </button>
